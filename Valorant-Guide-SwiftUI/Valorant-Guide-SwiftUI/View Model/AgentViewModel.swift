...
class AgentViewModel: ObservableObject {
    ...
    @Published var agentsData: (ResourceParser<AgentsParser>.ParsedType)?
    init() {
        guard let apiURL = baseURL else {
            fatalError("Failed to create baseURL")
        }
        apiHandler = APIClient(baseURL: apiURL, apiProvider: provider)
        apiHandler.fetchResourceData(resource: URLHeads.agents.rawValue, parser: parser, completion: { result in
            switch result {
